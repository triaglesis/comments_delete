...
def del_comments_regular(token, tk_owner_id, tk_group_id, tk_domain):
    ...
    # log_block_comments("Fast deleter - 30_sec Bot has started it's work")
    ...
    try:
        ...
        log_block_comments("Fast deleter - Error: unable to fecth data")
    ...
    post_id_list = []
    try:
        r_get_posts = requests.post('https://api.vk.com/method/wall.get?owner_id='+tk_owner_id+'&domain='+tk_domain+'&v=5.45&count=19&access_token='+token)
        ...
        if not result:
            ...
            log_block_comments("Fast deleter - "+str(result))
    except:
        log_block_comments("Fast deleter - ERROR!!! : Connection ERROR while get wall posts!")
    ...
    all_comments = []
    if post_id_list:
        # log_block_comments("Fast deleter - Get comments from posts: "+" "*26+str(len(post_id_list)))
        splited = split_by(post_id_list, 20)
        if len(splited) > 1:
            item_iteration = 1
            for item in splited:
                # log_block_comments("Fast deleter - Iterations were done: "+" "*26+str(item_iteration)+" of "+str(len(splited)))
                item_str = str(item)
                ...
                try:
                    ...
                    if not result:
                        ...
                        log_block_comments("Fast deleter - "+str(response_data))
                except:
                    log_block_comments("Fast deleter - ERROR!!! : Connection ERROR while wall_getComments_parsed!")
        else:
            for item in splited:
                item_str = str(item)
                post_list_arg = replace_all(item_str, reps)
        ...
        log_block_comments("Fast deleter - No posts found!")
    ...
    # log_block_comments(str(list_of_commet_tuples))
    # log_block_comments("Fast deleter - How many comments: "+" "*32+str(len(list_of_commet_tuples)))
    list_of_users = []
    ...
    if list_of_users:
        ...
        # log_block_comments("Fast deleter - How many users to check: "+" "*26+str(len(list_of_users)))
        if len(splited) > 1:
            item_iteration = 1
            for item in splited:
                # log_block_comments("Fast deleter - Iterations were done: "+" "*26+str(item_iteration)+" of "+str(len(splited)))
                ...
                try:
                    r = requests.post('https://api.vk.com/method/groups.isMember?group_id='+tk_group_id+'&v=5.45&user_ids='+user_list_arg+'&access_token='+token)
                    response_data = r.json()
                    result = [value for key, value in response_data.items() if key in error_keys]
                    if not result:
                        data = response_data['response']
                    ...
                    log_block_comments("Fast deleter - ERROR!!! : Connection ERROR while get isMember!")
        else:
            for item in splited:
                item_str = str(item)
                user_list_arg = replace_all(item_str, reps)
        ...
        log_block_comments("Fast deleter - List of members cannot be extracted")

    list_to_delete = []
    if all_members_not:
        log_block_comments("Fast deleter - all_members_not ARE: "+str(all_members_not))
        ...
        for each in list_of_commet_tuples:
            ...
            if user_id in members_silence:
                log_block_comments("Fast deleter - Will delete comment from silenced user: "+str(user_id))
                list_to_delete.append(each[0])
                comment_date = datetime.datetime.fromtimestamp(each[2]).strftime('%Y-%m-%d')  # Format time from UNIX to normal
                comment_time = datetime.datetime.fromtimestamp(each[2]).strftime('%H:%M:%S')  # Format time from UNIX to normal
                sql_comments = """INSERT INTO DELETED_COMMENTS(comment_id,comment_user_id,comment_date,comment_time,comment_likes)
        ...
        log_block_comments("Fast deleter - There are no extracted from isUser, stop work!")

    if list_to_delete:
        log_block_comments("Fast deleter - Bot will delete those comments: "+" "*26+str(len(list_to_delete)))
        ...
        if len(splited) > 1:
            ...
            for item in splited:
                ...
                try:
                    ...
                    log_block_comments("Fast deleter - ERROR!!! : Connection ERROR while deleteComments!")
        else:
            for item in splited:
                item_str = str(item)
                comm_list_arg = replace_all(item_str, reps)
                try:
                    ...
                    log_block_comments("Fast deleter - ERROR!!! : Connection ERROR while deleteComments!")
    # else:
    #     log_block_comments("Nothing to delete this time"+" "*24+"0")
        db.close()
        end = time()
        log_block_comments("Fast deleter - Bot is finishing this iteration! Time spent: "+" "*6+str((end - start)))
        log_block_comments("===========================================================================================")

# Instant run:
# logFile = 'comments_deleter_log.txt'
# delete_comments_older(logFile, token = '14de56889d3a53e072cb6b0823bff9c499fa064f6d057ae8b8f30845aeb2c9388b45e303131057259034c', tk_owner_id = "-46631810" , tk_group_id = "46631810", tk_domain = "typical_kirovohrad", offset=19)