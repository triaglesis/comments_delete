...
def del_comments_regular(log_file, token, tk_owner_id, tk_group_id, tk_domain):
    ...
    def log_block(message):
        with open(log_file, "a", encoding='utf-8') as f:
            f.write(curr_date+" 30_sec_bot -> "+message+"\n")
    ...
    try:
        ...
        log_block("Error: unable to fecth data")
    ...
    start = time()
    # print("30_sec Bot Comment after 30 sec sleep: "+str(curr_date))
    # log_block("30_sec Bot has started it's work")
    ...
    try:
        ...
        log_block("ERROR!!! : Connection ERROR while get wall posts!")
    ...
    all_comments = []
    if post_id_list:
        # log_block("Get comments from posts: "+" "*26+str(len(post_id_list)))
        splited = split_by(post_id_list, 20)
        if len(splited) > 1:
            item_iteration = 1
            for item in splited:
                # log_block("Iterations were done: "+" "*26+str(item_iteration)+" of "+str(len(splited)))
                item_str = str(item)
                ...
                try:
                    ...
                    if not result:
                        ...
                        log_block(response_data)
                except:
                    log_block("ERROR!!! : Connection ERROR while wall_getComments_parsed!")
        else:
            for item in splited:
                item_str = str(item)
                post_list_arg = replace_all(item_str, reps)
    ...
    # log_block(str(list_of_commet_tuples))
    # log_block("How many comments: "+" "*32+str(len(list_of_commet_tuples)))
    ...
    if list_of_users:
        ...
        # log_block("How many users to check: "+" "*26+str(len(list_of_users)))
        if len(splited) > 1:
            item_iteration = 1
            for item in splited:
                # log_block("Iterations were done: "+" "*26+str(item_iteration)+" of "+str(len(splited)))
                ...
                try:
                    ...
                    log_block("ERROR!!! : Connection ERROR while get isMember!")
        else:
            for item in splited:
                item_str = str(item)
                user_list_arg = replace_all(item_str, reps)
        ...
        log_block("List of members cannot be extracted")

    list_to_delete = []
    if all_members_not:
        log_block("all_members_not ARE: "+str(all_members_not))
        ...
        for each in list_of_commet_tuples:
            ...
            if user_id in members_silence:
                log_block("Will delete comment from silenced user: "+str(user_id))
                list_to_delete.append(each[0])
                comment_date = datetime.datetime.fromtimestamp(each[2]).strftime('%Y-%m-%d')  # Format time from UNIX to normal
                comment_time = datetime.datetime.fromtimestamp(each[2]).strftime('%H:%M:%S')  # Format time from UNIX to normal
                sql_comments = """INSERT INTO DELETED_COMMENTS(comment_id,comment_user_id,comment_date,comment_time,comment_likes)
        ...
        log_block("There are no extracted from isUser, stop work!")

    if list_to_delete:
        log_block("Bot will delete those comments: "+" "*26+str(len(list_to_delete)))
        ...
        if len(splited) > 1:
            ...
            for item in splited:
                ...
                try:
                    ...
                    log_block("ERROR!!! : Connection ERROR while deleteComments!")
        else:
            for item in splited:
                item_str = str(item)
                comm_list_arg = replace_all(item_str, reps)
    ...
    # else:
    #     log_block("Nothing to delete this time"+" "*24+"0")

        db.close()
        end = time()
        log_block("Bot is finishing this iteration! Time spent: "+" "*6+str((end - start)))
        log_block("===========================================================================================")
